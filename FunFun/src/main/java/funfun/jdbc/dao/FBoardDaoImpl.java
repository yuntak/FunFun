...
public class FBoardDaoImpl implements FBoardDao {
	...
	public List<FBoard> selectFBoardByPage(String code, int page_no) {
		String sql = "SELECT * FROM (SELECT sub.*, ROWNUM AS RNUM FROM ( select * from free_board order by board_no desc) sub) WHERE RNUM >= ? AND RNUM <= ? and board_code=?";
	...
	public List<FBoard> selectFBoardBySelectUserIdPage(String userId, String code, int page_no) {
		String sql = "SELECT * FROM (SELECT sub.*, ROWNUM AS RNUM FROM ( select * from free_board where user_id like '%'||?||'%' and board_code=? order by board_no desc) sub) WHERE RNUM >= ? AND RNUM <= ?";
	...
	public List<FBoard> selectFBoardBySelectTitlePage(String title, String code, int page_no) {
		String sql = "SELECT * FROM (SELECT sub.*, ROWNUM AS RNUM FROM ( select * from free_board where title like '%'||?||'%' and board_code=? order by board_no desc) sub) WHERE RNUM >= ? AND RNUM <= ?";
	...
	public int selectCount(int to, int from) {
		String sql = "SELECT count(*) FROM (SELECT sub.*, ROWNUM AS RNUM FROM ( select * from free_board order by board_no desc) sub) WHERE RNUM >= ? AND RNUM <= ?";
