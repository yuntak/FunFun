...
public class CBoard_subDaoImpl implements CBoard_subDao {
	...
	public int selectCountCno(int cno) {
		String sql = "select count(*) from (select c.*,cs.* from content_board c,content_board_sub cs where c.cno=cs.content_board_cno and cs.content_board_cno=?)";
	...
	public List<CBoard_sub> selectCBoardSubByPage(int cno, int page_no) {
		...
		List<CBoard_sub> result = jt.query(sql, new BeanPropertyRowMapper<CBoard_sub>(CBoard_sub.class), cno,
				(page_no - 1) * CBoard_subDao.BOARD_PER_PAGE + 1, page_no * CBoard_subDao.BOARD_PER_PAGE);
		return result;
	}
	...
	public List<Integer> selctCountAll(int cno) {
		String sql = "select count(*) from content_board_sub where content_board_cno=?";
		List<Integer> result = jt.queryForList(sql, Integer.class, cno);
		return result;
	}
